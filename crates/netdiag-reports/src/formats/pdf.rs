...
use genpdf::{
    ...
    fonts, style::{self, StyledString}, Alignment, Document, Element, Size,
};
...
impl PdfFormatter {
    ...
    pub fn generate_pdf(&self, report: &DiagnosticReport) -> ReportResult<Vec<u8>> {
        ...
        let font_family = fonts::from_files("./fonts", "LiberationSans", None)
            ...
            .or_else(|_| fonts::from_files("/opt/homebrew/share/fonts/liberation-sans", "LiberationSans", None))
            .or_else(|_| fonts::from_files("/usr/local/share/fonts/liberation-sans", "LiberationSans", None))
            ...
            .map_err(|_| ReportError::Formatting(
                "Failed to load fonts for PDF generation. \
                 ...
                 - Or place LiberationSans-*.ttf files in ./fonts/".to_string()
            ))?;
...
fn title_element(text: &str) -> impl Element {
    Paragraph::new(
        StyledString::new(text, style::Style::new().bold().with_font_size(20))
    ).aligned(Alignment::Center)
}

fn section_header(text: &str) -> impl Element {
    Paragraph::new(
        StyledString::new(text, style::Style::new().bold().with_font_size(14))
    )
}

fn subsection_header(text: &str) -> impl Element {
    Paragraph::new(
        StyledString::new(text, style::Style::new().bold().with_font_size(11))
    )
}

fn body_text(text: &str) -> impl Element {
    Paragraph::new(
        StyledString::new(text, style::Style::new().with_font_size(10))
    )
}

fn bold_text(text: &str, size: u8) -> impl Element {
    Paragraph::new(
        StyledString::new(text, style::Style::new().bold().with_font_size(size))
    )
}

fn centered_text(text: &str, size: u8) -> impl Element {
    Paragraph::new(
        StyledString::new(text, style::Style::new().with_font_size(size))
    ).aligned(Alignment::Center)
}

fn bullet_item(text: &str) -> impl Element {
    Paragraph::new(
        StyledString::new(format!("  - {}", text), style::Style::new().with_font_size(10))
    )
}
